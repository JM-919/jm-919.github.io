...
图片按 Markdown 标准写法即可，**本地图片会在构建时自动内联成 data URI**，文章页依旧是单文件自包含，
不需要额外上传图片：
...
- **建议**：单图控制在 200KB 以内（内联后 base64 约 +33%）。超过 1.5MB 构建时会提示。
- **想用原图外链**：也可以把图片放进 `assets/img/`，在 md 里用相对路径引用；
  `deploy_github.js` 会把 `assets/img/` 下的图片一并上传（页面就不再是纯自包含）。
